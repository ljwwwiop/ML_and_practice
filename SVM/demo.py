...
def loadDataSet(filename):
    ...
    加载数据
    :param filename: 文件名
    :return: dataMat 特征矩阵 labelMat 标签矩阵
    '''
    dataMat = []
    labelMat = []
    fr = open(filename)
...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    :param C:  松弛变量（常数值），允许有些数据点可以处于分娩的错误
                控制最大化间隔和保证大部分的函数间隔小于1.0这连哥目标的权重
                可以通过调节该参数达到不同的结果
    :param toler: 容错率（一些因素或选择对某个系统产生不稳定的概率）
    :param maxIter:  退出前最大的循环次数
    :return: b  模型的常量值  alphas 拉格朗日乘数
    '''
